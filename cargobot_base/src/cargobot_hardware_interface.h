...
#ifndef CARGOBOT_HARDWARE_INTERFACE_HPP_
#define CARGOBOT_HARDWARE_INTERFACE_HPP_
...
class CargoBotHardwareInterface : public hardware_interface::RobotHW {
public:
	CargoBotHardwareInterface(ros::NodeHandle node, ros::NodeHandle private_node, double target_max_wheel_angular_speed);
...
CargoBotHardwareInterface::CargoBotHardwareInterface(ros::NodeHandle node, ros::NodeHandle private_node, double target_max_wheel_angular_speed)
	: _node(node)
	...
	_left_wheel_angle_sub = _node.subscribe("/left_wheel/angle", 1, &CargoBotHardwareInterface::frontLeftWheelAngleCallback, this);
	_right_wheel_angle_sub = _node.subscribe("/right_wheel/angle", 1, &CargoBotHardwareInterface::frontRightWheelAngleCallback, this);
}

void CargoBotHardwareInterface::writeCommandsToHardware() {
...
void CargoBotHardwareInterface::updateJointsFromHardware(const ros::Duration& period) {
...
void CargoBotHardwareInterface::registerControlInterfaces() {
	ros::V_string joint_names = boost::assign::list_of("base_to_left_wheel")("base_to_right_wheel");
...
void CargoBotHardwareInterface::frontLeftWheelAngleCallback(const std_msgs::Float64& msg) {
	_left_wheel_angle = msg.data;
}

void CargoBotHardwareInterface::frontRightWheelAngleCallback(const std_msgs::Float64& msg) {
	_right_wheel_angle = msg.data;
}

void CargoBotHardwareInterface::limitDifferentialSpeed(double& diff_speed_left_side, double& diff_speed_right_side) {
...
#endif // CARGOBOT_HARDWARE_INTERFACE_HPP_
